...
- CPU 사용률 99~100%로 포화 → 이미지 처리 과정의 CPU-intensive 작업이 병목으로 작용
...
- 측정 결과, **이미지 업로드 단계(upload_images)가 가장 큰 병목**임을 확인했습니다.
## 4.3. Thread Pool 확장 시도

처음에는 단순히 **스레드 풀 크기를 늘리면** 병목이 완화될 거라 생각했습니다.(바보같이 I/O bound라 판단함)
...
CPU-intensive 작업이 병목인 상태에서 스레드 수를 증가시키면서 **CPU 자원 경쟁**과 **컨텍스트 스위칭 오버헤드**가 증가한 것이 원인으로 판단된다.
...
- **이미지 업로드 & Redis 메시지 발행은 백그라운드에서 처리**  
    → HTTP 요청 스레드의 블로킹이 제거되면서 요청 처리 구조가 개선되고, CPU 사용 패턴이 안정화 되었다.
...
> 업로드 시간을 줄이기 위해 **이미지 최적화(webp 변환/리사이징)** 를 제거하고 테스트했다.
...
- 이미지 최적화 로직 제거로 데이터 크기가 증가하면서 **네트워크 전송 시간과 디스크 I/O 대기시간**이 증가
- CPU는 가벼워졌지만, 전체 응답은 더 느려짐
- CPU와 I/O는 trade-off 관계이며, CPU 부담을 줄이면 I/O가 새로운 병목으로 드러날 수 있음을 확인함.
## 6.3. 결론
...
단일 서버 환경에서는 비동기 처리를 통해 CPU 사용 패턴을 개선할 수 있었지만, 근본적으로는 **무거운 I/O 작업이 API서버에 집중되는 구조적 한계**라고 생각한다.

기존 구조:
- API 서버(Spring)이 요청 처리 + 이미지 다운로드 + 파일 저장을 모두 수행
- 네트워크 I/O와 디스크 I/O가 API 서버 자원을 직접 점유
- 트래픽 증가 시 서버 부하가 선형적으로 증가

개선 방향:

> 무거운 I/O 작업을 API 서버에서 분리하고, 별도의 실행 환경에서 처리하도록 설계

1. FastAPI → 영상의 임시 URL 반환
2. Lambda
	- 영상 다운로드(Streaming 방식)
	- S3 업로드
3.  MySQL → 최종 영상 URL 저장

기대 효과:
1. 서버 부하 분리
	- API 서버는 요청 처리에만 집중
	- 무거운 I/O 작업은 별도 실행 환경에서 처리
2. 확장성 확보(수평적 확장)
	- 요청 증가 시 작업 처리 영역이 독립적으로 확장
	- API 서버와 I/O 처리 영역이 분리됨
3. 구조적 개선
	- CPU-intensive 작업과 I/O 작업을 분리
	- 병목이 특정 서버에 집중되지 않도록 개선

> 단순히 비동기로 처리에 그치말고, C**PU 작업과 I/O 작업을 구조적으로 분리하는 것**이 근본적인 해결책이라고 생각한다.