---
category: Project/01_뽀송길
---
### 1. 개요
1.1 [[1_프로젝트_소개|프로젝트 소개]]
1.2 [[2_초단기예보_초단기실황|초단기예보 & 초단기실황]]
1.3 [[3_대중교통_경로|대중교통 경로]]
1.4 [[4_OSRM|OSRM]]
1.5 [[5_결과|결과]]
### 2. 트러블슈팅
2.1 [[OSRM_실행과정_일대기|OSRM 실행과정 일대기]]
	2.1.1 [[서울시_지리공간_데이터_만들기|서울시 지리공간 데이터 만들기]]
2.2 [[뽀송_가중치_설정_과정|뽀송 가중치 설정 과정]]
2.3 [[Docker_Springboot_환경설정_개선기|Docker & Spring Boot 환경설정 개선기]]
2.4 [[최단경로탐색_알고리즘|최단경로 탐색 알고리즘]]
2.5 [[Promise_all을_이용한_기상_데이터_수집_개선기|Promis all을 이용한 기상 데이터 수집 개선기]]
### 3. 산출물
3.1 [뽀송길 프로젝트 보고서](https://drive.google.com/file/d/1wrz1E6c7A9nHXzTyQL8pcUO7h0W5dfZO/view?usp=sharing)
3.2 [뽀송길_경진대회_보고서](https://drive.google.com/file/d/1k3UiB7mK23dokLBPTBnj6_33Zd_6Vqqt/view?usp=sharing)
3.3 [[발표자료|발표자료]]
3.4 [시연영상](https://youtu.be/pW2QbOUD66s)
3.5 [GitHub 주소](https://github.com/Gonagi/pposonggil_v2)
### 4. 특허
4.1 [특허 명세서](https://drive.google.com/file/d/1BqbH9mqQPMe8iml9xVuBqoSrhSbSwR9L/view?usp=drive_link)